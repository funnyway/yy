<?php /* V2.10 Template Lite 4 January 2007  (c) 2005-2007 Mark Dickenson. All rights reserved. Released LGPL. 2015-04-13 13:43 ?D1ú±ê×?ê±?? */  $_templatelite_tpl_vars = $this->_vars;
...
" maxlength="10"  onkeyup="if(event.keyCode !=37 && event.keyCode != 39) value=value.replace(/\D/g,'');" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/\D/g,''))"/>        
		...
		<tr>
		  ...
		  <td><input name="per_refresh_resume_space" type="text"  class="input_text_150"   value="<?php echo $this->_vars['config']['per_refresh_resume_space']; ?>
" maxlength="10"  onkeyup="if(event.keyCode !=37 && event.keyCode != 39) value=value.replace(/\D/g,'');" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/\D/g,''))"/> 分钟
			<span class="admin_note">0为不限制</span>
		  </td>
		</tr>
		<tr>
		  <td align="right">每天刷新简历次数限制：</td>
		  <td><input name="per_refresh_resume_time" type="text"  class="input_text_150"   value="<?php echo $this->_vars['config']['per_refresh_resume_time']; ?>
" maxlength="10"  onkeyup="if(event.keyCode !=37 && event.keyCode != 39) value=value.replace(/\D/g,'');" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/\D/g,''))"/> 次
			<span class="admin_note">0为不限制</span>
		  </td>
		</tr>
		<tr>
...
<input type="radio" name="showresumecontact" value="2"  <?php if ($this->_vars['config']['showresumecontact'] == "2"): ?>checked=checked<?php endif; ?>/>下载后可见
 ...
 <span class="admin_note">(如此项为“游客可见”或“已登录会员可见”，网站将会失去重要赢利点)</span></td>
	...
	<tr>
      <td width="200" align="right">企业查看收到的简历无需下载：</td>
      <td><label>
        <input name="showapplycontact" type="radio" value="0"  <?php if ($this->_vars['config']['showapplycontact'] == "0"): ?>checked=checked <?php endif; ?>/>关闭</label>
&nbsp;&nbsp;&nbsp;&nbsp;
<label >
<input type="radio" name="showapplycontact" value="1"  <?php if ($this->_vars['config']['showapplycontact'] == "1"): ?>checked=checked<?php endif; ?>/>开启
 </label></td>
    </tr>
	<tr>
	  <td align="right" valign="top">&nbsp;</td>
...
<input type="radio" name="elite_resume_complete_percent" value="100"    <?php if ($this->_vars['config']['elite_resume_complete_percent'] == "100"): ?>checked=checked<?php endif; ?>/>100%
      ...
      <td>
	  <textarea name="personal_talent_requirement" style="width:700px; height:160px; font-size:12px; line-height:180%"  id="personal_talent_requirement"><?php echo $this->_vars['text']['personal_talent_requirement']; ?>
</textarea>
	   <script>
			var editor;
			KindEditor.ready(function(K) {
				editor = K.create('textarea[name="personal_talent_requirement"]', {
